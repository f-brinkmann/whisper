...
% transformed staircase procedure following a 2-down/1-up rule, halving step
...
% [x_next, x_est]
%
% = staircase2d1u(x_hist, r_hist, range, nrmax, halvings, numlastr)
%
% 
% -------------------------------------------------------------------------
% 
...
%                     Trial einer 2-down/1-up Regel folgend. 
%
...
function [x_next, x_est] = staircase2d1u(x_hist, r_hist, range, nrmax, halvings, numlastr)
...
% (2-Down/1-UP)
if (Ntrials>=2) 
    if (T>=2) && (r_hist(Ntrials) == 1) && (r_hist(Ntrials-1) == 1)
        this_change = -1;
