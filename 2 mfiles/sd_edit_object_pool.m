...
function varargout = sd_edit_object_pool(varargin)
% SD_EDIT_OBJECT_POOL M-file for sd_edit_object_pool.fig
%      SD_EDIT_OBJECT_POOL, by itself, creates a new SD_EDIT_OBJECT_POOL or raises the existing
%      singleton*.
%
%      H = SD_EDIT_OBJECT_POOL returns the handle to a new SD_EDIT_OBJECT_POOL or the handle to
%      the existing singleton*.
%
%      SD_EDIT_OBJECT_POOL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SD_EDIT_OBJECT_POOL.M with the given input arguments.
%
%      SD_EDIT_OBJECT_POOL('Property','Value',...) creates a new SD_EDIT_OBJECT_POOL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sd_edit_object_pool_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sd_edit_object_pool_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sd_edit_object_pool
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @sd_edit_object_pool_OpeningFcn, ...
                   'gui_OutputFcn',  @sd_edit_object_pool_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before sd_edit_object_pool is made visible.
function sd_edit_object_pool_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sd_edit_object_pool (see VARARGIN)

% Choose default command line output for sd_edit_object_pool
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes sd_edit_object_pool wait for user response (see UIRESUME)
% uiwait(handles.fig_edit_element_pool);

movegui(hObject,'center')
global TSP PP
load ([PP.path filesep 'TSP.mat']);
set(hObject,'Name',['SD - object pool (' TSP.sections{PP.edit_section_line,2} ')'])
...
function varargout = sd_edit_object_pool_OutputFcn(hObject, eventdata, handles) 
