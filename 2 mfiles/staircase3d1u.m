...
% transformed staircase procedure following a 3-down/1-up rule, halving step
% size after predefined reversals and estimating the threshold by calcula-
% ting "midrun estimates" for for a predetermined number of last consecutive 
% reversals.
%
%
% bibliography:
...
% [x_next, x_est]
%
% = staircase3d1u(x_hist, r_hist, range, nrmax, halvings, numlastr)
%
% 
% -------------------------------------------------------------------------
% 
...
%                     Trial einer 3-down/1-up Regel folgend. 
%
...
function [x_next, x_est] = staircase3d1u(x_hist, r_hist, range, nrmax, halvings, numlastr)
...
T = 1; % Zähler für Trials bei gleichbleibender Reizstärke
x_est = 0;
...
% (3-Down/1-UP)
if (Ntrials>=3) 
    if (T>=3) && (r_hist(Ntrials) == 1) && (r_hist(Ntrials-1) == 1) && (r_hist(Ntrials-2) == 1)
        this_change = -1;
    elseif r_hist(Ntrials) == -1
        this_change = +1;
    else
    this_change = 0;
    end
...
% Reversal?
if (this_change ~= 0) && (this_change ~= latest_change) && (latest_change ~=0) 
    nr = nr + 1;
    x_r(nr)=x_hist(Ntrials);
    % ggf. Halbierung der Schrittweite
    if isempty(find(halvings==nr))==0
           numhalv = numhalv + 1;
    end
end
